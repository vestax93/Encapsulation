...
public class Dough {
    ...
    private void validateFlourType(String flourType) {
        boolean isValid = flourType.equals(FLOUR_WHITE) || flourType.equals(FLOUR_WHOLEGRAIN); //provera da li je tip brasna u okviru predvidjenih vrednosti
        if(!isValid){
    ...
    private void validateBakingTechnique(String bakingTechnique) {
        boolean isValid = bakingTechnique.equals(BAKING_TECH_CHEWY) || bakingTechnique.equals(BAKING_TECH_CRISPY)
                || bakingTechnique.equals(BAKING_TECH_HOMEMADE); //provera da li je tip pecenja u okviru predvidjenih vrednosti
        if(!isValid){
            throw new IllegalArgumentException("Invalid type of dough.");
        }
    }
    ...
    public double calculateCalories(){
        double calories = BASE_CALORIES * this.weight *
                this.getModifierValue(this.flourType) * this.getModifierValue(this.bakingTechnique);
        return calories;
    }

    private double getModifierValue(String modifier) { //odredjivanje vrednosti kalorija u odnosu na vrstu pecenja/brasna
